...
class MetricDef:
    ...
    key: str
    label: str
    expression: str  # e.g. "uniqExact(user_id)" or "sum(value)"
    table: str
    event_filter: Optional[str] = None  # when querying funnel_events
    description: str = ""
...
# Shared funnel table for counts / users / funnel stages
_FUNNEL = "funnel_events"
_PURCHASE = "purchase_completed"
...
METRICS: dict[str, MetricDef] = {
    "revenue": MetricDef(
        ...
        table=_PURCHASE,
        description="Sum of purchase value",
    ),
    "purchases": MetricDef(
        key="purchases",
        label="Purchases",
    ...
    "conversion_rate": MetricDef(
        ...
        expression=(
            "uniqExactIf(user_id, event = 'purchase_completed') "
            "/ nullIf(uniqExactIf(user_id, event = 'application_started'), 0)"
        ),
        table=_FUNNEL,
...
def list_dimensions() -> list[dict[str, str]]:
    ...
    for d in DIMENSIONS.values():
        ...
        if d.key in {"geoip_country_code"} and "country" in DIMENSIONS:
            continue
        seen.add(d.column)
...
def catalog_prompt_blurb() -> str:
    ...
    return (
        ...
        "Aliases: country→geoip_country_code, channel→device_type.\n"
